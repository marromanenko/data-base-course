            ...
            if choice == '0':
                ...
                while not end_insert:
                    try:
                        ...
                        if table == 1:
                            ...
                            usid = int(input('User ID = '))
                            Model.insert_for_table1(usid, usname, usbirth_date)
                        elif table == 2:
                            poid = int(input('ID = '))
                            poname = input('Name of the post = ')
                            potopic = input('Topic = ')
                            pouser = int(input('User ID = '))
                            Model.insert_for_table2(poid, poname, potopic, pouser)
                        elif table == 3:
                            coid = int(input('Comment ID = '))
                            codate = input('Date = ')
                            cotext = input('Text = ')
                            copost = int(input('PostID = '))
                            Model.insert_for_table3(coid, codate, cotext, copost)
                        else:
                            print('Incorrect input, try again.')
                    except:
                        print("such a primary key already exists or there is no such secondary key")
                    incorrect = True
                    while incorrect:
                ...
                while not end_delete:
                    try:
                        ...
                        print("there are some dependence, you can't delete this item")
                    incorrect = True
                    while incorrect:
                ...
                while not end_update:
                    try:
                        ...
                        table = Model.validTable(table)
                        if table == 1:
                            usid = int(input('Attribute to update(where) User ID = '))
                            usname = input("New name = ")
                            usbirth_date = input("New birth date = ")
                            Model.update_for_table1(usid, usname, usbirth_date)
                        elif table == 2:
                            poid = int(input('Attribute to update(where) Post ID = '))
                            ponamepost = input("New namepost = ")
                            potopic = input("New topic = ")
                            Model.update_for_table2(poid, ponamepost, potopic)
                        elif table == 3:
                            coid = int(input('Attribute to update(where) Comment ID = '))
                            codate = input("New date = ")
                            cotext = input("New text = ")
                            Model.update_for_table3(coid, codate, cotext)
                        else:
                            print('Incorrect input, try again.')
                    except:
                        print("this ID isn't exist")
                    incorrect = True
                    while incorrect:
